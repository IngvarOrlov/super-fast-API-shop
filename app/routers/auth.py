...
@router.get('/users/me')
async def read_current_user(user: dict = Depends(authenticate_user)):
    return {'User': user}


@router.post('/', status_code=status.HTTP_201_CREATED)
async def create_user(session: Annotated[AsyncSession, Depends(get_db)], create_user: CreateUser):
    await session.execute(insert(User).values(first_name=create_user.first_name,
...
async def get_supplier_or_admin_user(user: Annotated[dict, Depends(get_current_user)]):
    if not user.get('is_supplier') and not user.get('is_admin'):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
